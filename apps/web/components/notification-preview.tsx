...
export type RaceAnnouncementPreview = {
  eventType: 'RACE_ANNOUNCED';
  generatedAt: string;
  plannedAt: string;
...
export function NotificationPreview({ preview, action, busy = false }: { preview: RaceAnnouncementPreview; action?: { label: string; onClick: () => void }; busy?: boolean }) {
  const channels = [
  ...
  return <section className="delivery-preview" aria-label={`${preview.race.name}の配信前確認`}>
    <div className="delivery-preview-heading"><div><span className="eyebrow">DELIVERY PREVIEW</span><h4>配信前確認</h4></div><span className="status-tag">告知 第{preview.version}版</span></div>
    ...
    {action && <button className="button delivery-preview-action" disabled={busy} onClick={action.onClick}>{busy ? '処理中…' : action.label}</button>}
