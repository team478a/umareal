...
type Selection = { entryId: string; horseId: string; number: number; horseName: string; status: string; selectionType: string };
type Paper = { product: { expertName: string; confidence: string; summary: string }; races: { legNumber: number; confidence: string; strategyType: string; comment: string; race: RaceMeta & { raceDate: string; name: string }; selections: Selection[] }[]; combinationCount: number; amountPerPointYen: number; assumedPurchaseAmountYen: number };
type Detail = { access: 'FULL' | 'METADATA'; product: Product; version: (VersionMeta & { confidence: string; combinationCount: number; amountPerPointYen: number; assumedPurchaseAmountYen: number; contentSnapshot: Paper; deadlineAt: string; correctionReason: string | null }) | null; versions: VersionMeta[]; locked: boolean };
...
export function Win5Paper({ productId, loggedIn }: { productId: string; loggedIn: boolean }) {
  ...
  return <><div className="page-heading"><span className="eyebrow">WIN5 PAPER · {product.targetDate}</span><h1>{product.title}</h1><p>{version ? `${version.status === 'CORRECTED' ? '訂正版' : '初版'} v${version.version}・${dateTime(version.publishedAt)} JST 公開` : `${dateTime(product.scheduledPublishAt)} JST 公開予定`}</p></div>
    ...
    {detail.locked && <section className="panel win5-lock"><LockKeyhole size={30} /><div><h2>選択馬・根拠・想定金額は有料会員向けです</h2><p>月額会員または対象日の1日利用で、公開済み紙面の本文と訂正履歴を確認できます。</p></div><Link className="button" href={loggedIn ? `/plans?date=${product.targetDate}` : `/login?next=/win5/${product.id}`}>{loggedIn ? '閲覧プランを確認' : 'ログイン'}<ArrowRight size={16} /></Link></section>}
...
function PaperBody({ version }: { version: NonNullable<Detail['version']> }) {
  ...
  const strategy: Record<string, string> = { NARROW: '絞る', NORMAL: '標準', SPREAD: '広げる' };
  return <section className="win5-paper-sheet"><header><div><span>WIN5 PREVIEW</span><h2>{paper.product.expertName}の5レース紙面</h2></div><div><small>全体信頼度</small><strong>{paper.product.confidence}</strong></div></header><div className="win5-paper-legs">{paper.races.map(leg => <article key={leg.legNumber}><div className="win5-paper-race"><b>第{leg.legNumber}レース</b><div><strong>{leg.race.venue} {leg.race.number}R · {leg.race.name}</strong><small>{dateTime(leg.race.startsAt)} JST</small></div><span>信頼度 {leg.confidence}</span></div><div className="win5-selections">{leg.selections.map(selection => <div className={selection.selectionType === 'CENTER' ? 'center' : ''} key={selection.entryId}><span>{selection.selectionType === 'CENTER' ? '中心' : '相手'}</span><b>{selection.number}</b><strong>{selection.horseName}</strong></div>)}</div><div className="win5-comment"><span>戦略：{strategy[leg.strategyType] ?? leg.strategyType}</span><p>{leg.comment}</p></div></article>)}</div><footer><dl><div><dt>組み合わせ数</dt><dd>{version.combinationCount}点</dd></div><div><dt>1点あたり</dt><dd>{yen(version.amountPerPointYen)}</dd></div><div><dt>想定購入総額</dt><dd>{yen(version.assumedPurchaseAmountYen)}</dd></div></dl><div className="win5-summary"><span>全体総評</span><p>{paper.product.summary}</p></div>{version.correctionReason && <p className="win5-correction">訂正理由：{version.correctionReason}</p>}<small>掲載内容は情報提供を目的とし、購入・的中・利益を保証するものではありません。</small></footer></section>;
}
