...
export function ReferralDashboard({ onAccessChanged }: { onAccessChanged: () => Promise<void> }) {
  ...
  return <section className="panel referral-panel" aria-labelledby="referral-title">
    <div className="panel-heading"><div><span className="eyebrow">FRIEND REFERRAL</span><h2 id="referral-title">友達紹介</h2></div><Gift size={23} /></div>
    <div className="referral-hero"><div><span>現在の紹介成立</span><strong>{data.qualifiedCount}<small>人</small></strong></div><div><strong>{data.nextMilestone ? `あと${data.nextMilestone.remaining}人で一日券` : 'すべての特典を達成済み'}</strong><div className="referral-meter" aria-label={`${data.qualifiedCount} / ${nextTotal}`}><span style={{ width: `${Math.min(100, data.qualifiedCount / nextTotal * 100)}%` }} /></div><small>{data.qualifiedCount} / {nextTotal}</small></div></div>
    ...
    <div className="referral-milestones">{data.milestones.map(item => <article className={item.achieved ? 'achieved' : ''} key={item.id}><span>{item.achieved ? <Check size={18} /> : <Users size={18} />}</span><div><strong>{item.requiredReferralCount}人</strong><small>一日券 ×{item.rewardQuantity}</small></div><b>{item.achieved ? '達成' : '未達成'}</b></article>)}</div>
    <div className="referral-rewards"><h3>獲得した紹介特典</h3>{available.length ? available.map(reward => <form className="reward-card" key={reward.id} onSubmit={event => void redeem(event, reward)}><TicketCheck size={22} /><div><strong>未使用一日券</strong><small>{reward.milestone.requiredReferralCount}人達成特典 · {dateText(reward.expiresAt)}まで</small></div><label>利用日<input type="date" min={today()} max={jstDay(reward.expiresAt)} value={dates[reward.id] ?? today()} onChange={event => setDates({ ...dates, [reward.id]: event.target.value })} required /></label><button className="button small" disabled={busy === reward.id}>{busy === reward.id ? '処理中…' : 'この日に使う'}</button></form>) : <p className="muted">未使用の一日券はありません。</p>}
