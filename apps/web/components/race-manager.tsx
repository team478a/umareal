...
import { ArrowRight, FileUp, Plus, RefreshCw } from 'lucide-react';
...
export function RaceManager() {
  ...
  async function announce(race: Race) { const reason = announcementReasons[race.id]?.trim(); if (!reason) { setError('告知理由を入力してください。'); return; } setBusy(true); setError(''); setMessage(''); try { const result = await request<{ version: number }>(`races/${race.id}/announce`, 'POST', { reason }); setAnnouncementReasons({ ...announcementReasons, [race.id]: '' }); setMessage(`${race.venue} ${race.number}Rを告知しました（第${result.version}版）。`); await load(); } catch (e) { setError((e as Error).message); } finally { setBusy(false); } }
  ...
  return <>
    ...
    <section className="panel"><div className="panel-heading race-toolbar"><h2>登録レース</h2><label className="date-filter">表示する開催日<input aria-label="表示する開催日" type="date" value={date} onChange={e => { setDate(e.target.value); setPage(1); setEditing(false); setSelected(null); }} /></label><button className="button" onClick={() => { setSelected(null); setEntry(null); setEditing(true); }}><Plus size={16} />レースを追加</button></div>
      {races.length > 0 && <div className="announcement-quick-list"><div className="quick-list-heading"><strong>対象レース告知</strong><small>スマートフォンから理由を入力して公開できます。</small></div>{races.map(race => <div className="announcement-quick-row" key={race.id}><div><strong>{race.venue} {race.number}R {race.name}</strong><small>{race.announcements?.[0] ? `告知済み・第${race.announcements[0].version}版` : '未告知'}</small></div><label className="field"><span className="sr-only">{race.name}の告知理由</span><input aria-label={`${race.name}の告知理由`} value={announcementReasons[race.id] ?? ''} maxLength={500} onChange={event => setAnnouncementReasons({ ...announcementReasons, [race.id]: event.target.value })} placeholder="対象レースとして決定" /></label><button className="button small" disabled={busy || !(announcementReasons[race.id] ?? '').trim()} onClick={() => void announce(race)}>{race.announcements?.[0] ? '再告知' : '告知する'}</button></div>)}</div>}
