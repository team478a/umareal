...
type Item = { id: string; eventType: string; title: string; createdAt: string; publishedAt: string; version: number; visibility: string; readAt: string | null; race: { id: string; raceDate: string; venue: string; number: number; name: string; startsAt: string } };
...
export function MemberNotifications() {
  ...
  async function open(item: Item) { setBusy(item.id); setError(''); try { if (!item.readAt) await api(`me/notifications/${item.id}/read`, 'POST'); router.push(`/races/${item.race.id}`); } catch (e) { setError((e as Error).message); setBusy(''); } }
  return <><div className="page-heading"><span className="eyebrow">NOTIFICATIONS</span><h1>お知らせ</h1><p>対象レース、無料速報、最終予想の公開履歴を確認できます。</p></div>
    ...
    <section className="panel member-notifications"><div className="panel-heading notification-heading"><div><h2>配信履歴</h2><small>{data?.unreadCount ?? 0}件の未読</small></div><div className="notification-tabs" role="group" aria-label="お知らせの表示"><button className={!unread ? 'selected' : ''} aria-pressed={!unread} onClick={() => { setUnread(false); setPage(1); }}>すべて</button><button className={unread ? 'selected' : ''} aria-pressed={unread} onClick={() => { setUnread(true); setPage(1); }}>未読</button></div></div>
      {!data ? <div className="panel-body loading" role="status">お知らせを読み込み中…</div> : data.items.length === 0 ? <div className="empty"><BellRing size={32} strokeWidth={1.3} /><h3>{unread ? '未読のお知らせはありません' : 'お知らせはまだありません'}</h3><p>{unread ? 'すべて確認済みです。' : '対象レースの告知や予想公開後に表示されます。'}</p></div> : <div className="member-notification-list">{data.items.map(item => <article className={`member-notification ${item.readAt ? 'read' : 'unread'}`} key={item.id}><span className="notification-dot" aria-label={item.readAt ? '既読' : '未読'}>{item.readAt ? <Check size={14} /> : null}</span><div className="member-notification-copy"><div><span className="eyebrow">{item.eventType === 'RACE_ANNOUNCED' ? 'RACE ANNOUNCEMENT' : item.eventType.startsWith('FREE_REPORT_') ? 'FREE PADDOCK REPORT' : item.visibility === 'PAID' ? 'MEMBERS ONLY' : 'PREDICTION'}</span>{!item.readAt && <span className="status-tag">新着</span>}</div><h3>{item.title}</h3><strong>{item.race.venue} {item.race.number}R {item.race.name}</strong><small>{item.race.raceDate} · {formatDate(item.publishedAt)} JST · 第{item.version}版</small></div><button className="button secondary small" disabled={busy === item.id} onClick={() => void open(item)}>{busy === item.id ? '移動中…' : '詳細を見る'}<ChevronRight size={16} /></button></article>)}</div>}
