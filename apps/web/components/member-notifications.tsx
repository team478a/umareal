...
export function MemberNotifications() {
  ...
  return <><div className="page-heading"><span className="eyebrow">NOTIFICATIONS</span><h1>お知らせ</h1><p>対象レース、予想、評価結果、お支払い、お問い合わせ回答の履歴を確認できます。</p></div>
    ...
    <section className="panel member-notifications"><div className="panel-heading notification-heading"><div><h2>配信履歴</h2><small>{data?.unreadCount ?? 0}件の未読</small></div><div className="notification-tabs" role="group" aria-label="お知らせの表示"><button className={!unread ? 'selected' : ''} aria-pressed={!unread} onClick={() => { setUnread(false); setPage(1); }}>すべて</button><button className={unread ? 'selected' : ''} aria-pressed={unread} onClick={() => { setUnread(true); setPage(1); }}>未読</button></div></div>
      {!data ? <div className="panel-body loading" role="status">お知らせを読み込み中…</div> : data.items.length === 0 ? <div className="empty"><BellRing size={32} strokeWidth={1.3} /><h3>{unread ? '未読のお知らせはありません' : 'お知らせはまだありません'}</h3><p>{unread ? 'すべて確認済みです。' : '対象レースの告知、予想公開、評価結果、お支払い、お問い合わせ回答が表示されます。'}</p></div> : <div className="member-notification-list">{data.items.map(item => { const label = item.billing ? item.billing.planCode === 'DAY_PASS' ? `${item.billing.raceDate ?? ''} 1日利用` : item.billing.planCode === 'FOUNDER' ? '創設会員' : '通常月額会員' : null; return <article className={`member-notification ${item.readAt ? 'read' : 'unread'}`} key={item.id}><span className="notification-dot" aria-label={item.readAt ? '既読' : '未読'}>{item.readAt ? <Check size={14} /> : null}</span><div className="member-notification-copy"><div><span className="eyebrow">{item.billing ? 'BILLING' : item.support ? 'SUPPORT' : item.eventType.endsWith('EVALUATION_CONFIRMED') ? 'EVALUATION RESULT' : item.win5 ? 'WIN5 PAPER' : item.eventType === 'RACE_ANNOUNCED' ? 'RACE ANNOUNCEMENT' : item.eventType.startsWith('FREE_REPORT_') ? 'FREE PADDOCK REPORT' : item.visibility === 'PAID' ? 'MEMBERS ONLY' : 'PREDICTION'}</span>{!item.readAt && <span className="status-tag">新着</span>}</div><h3>{item.title}</h3><strong>{label ?? item.support?.subject ?? (item.win5 ? item.win5.title : `${item.race!.venue} ${item.race!.number}R ${item.race!.name}`)}</strong><small>{item.billing || item.support ? `${formatDate(item.publishedAt)} JST` : `${item.win5?.targetDate ?? item.race!.raceDate} · ${formatDate(item.publishedAt)} JST · 第${item.version}版`}</small></div><button className="button secondary small" disabled={busy === item.id} onClick={() => void open(item)}>{busy === item.id ? '移動中…' : '詳細を見る'}<ChevronRight size={16} /></button></article>; })}</div>}
