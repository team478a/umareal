...
import { ArrowDownRight, ArrowRight, ArrowUpRight, Eye, Headphones, Mic, PlayCircle, Square, Upload } from 'lucide-react';
...
export function FreeReportManager() {
  ...
  const [reason, setReason] = useState(''); const [benefitReason, setBenefitReason] = useState(''); const [error, setError] = useState(''); const [message, setMessage] = useState(''); const [busy, setBusy] = useState(false); const [preview, setPreview] = useState<NotificationPreviewData | null>(null);
  ...
  async function loadPreview(kind: 'PRE_RACE' | 'POST_RACE_REVIEW') { if (!detail) return; setBusy(true); setError(''); setMessage(''); try { setPreview(await api<NotificationPreviewData>(`admin/notifications/previews/free-report?raceId=${detail.id}&kind=${kind}&revision=${draft.revision}`)); } catch (e) { setError((e as Error).message); } finally { setBusy(false); } }
