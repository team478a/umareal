...
async function candidates(db: PrismaClient, now: Date): Promise<AlertCandidate[]> {
  ...
  const [failedDeliveries, failedSchedules, missedRaces] = await Promise.all([
...
async function detect(db: PrismaClient, now: Date) {
  ...
  for (const item of found) {
    await db.operationalAlert.upsert({
      where: { dedupeKey: item.dedupeKey },
      create: { ...item, detectedAt: now, lastObservedAt: now },
...
async function claim(db: PrismaClient, now: Date, limit: number) {
...
export async function runOperationalAlerts(input: { db: PrismaClient; transport?: OperationalAlertTransport; now?: () => Date; limit?: number }) {
  ...
  const leased = await claim(input.db, now(), limit); result.claimed = leased.ids.length;
