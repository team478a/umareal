...
- 配信予約・アラート: `/admin/publication-schedules`。対象レース告知と無料パドック速報は、対象人数、チャネル、本文、予定時刻を確認してから予約できます。公開後はレースと公開版ごとにWeb掲載日時、LINE・メールの送信済み／待機／失敗／対象外件数を確認し、受信者別の通知詳細へ移動できます。
