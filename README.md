...
- 運用・連携設定: `/admin/settings`。管理者が二段階認証後に、新規会員登録、LINE資格情報、再試行方針、予想公開・CSV取込・通知・新規購入の停止状態を管理します。新規登録停止時は会員向け案内を必須とし、既存会員のログインは維持します。秘密値は保存後に再表示しません。
