...
LINE・Stripeへのライブ疎通、正式結果データ取込、月額・利用開始後の返金条件確定、CMSは後続フェーズです。本番サービスとしては未完成です。
...
- 料金・契約: `/plans` で内容を確認してから申込、`/account` で契約・支払履歴、Stripe領収書、解約予約と取消、支払方法変更、請求問い合わせを確認できます。`/admin/billing` では請求問い合わせ、契約、支払済み・権限未付与Checkoutの再検証/全額返金、期限切れ公開待ち一日券の返金、支払失敗・回復のローカル試験を扱います。価格と販売可否は `/admin/settings` で管理します。
