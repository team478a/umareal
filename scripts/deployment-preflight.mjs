...
export function validateDeploymentEnvironment(service, env) {
  ...
  if (service === 'web') {
    ...
    return { service, launchMode: null, ok: errors.length === 0, errors, manual };
  }

  const launchMode = env.LAUNCH_MODE;
  requireValue('LAUNCH_MODE', ['FREE_REGISTRATION', 'FULL'].includes(launchMode), 'LAUNCH_MODE must be FREE_REGISTRATION or FULL.');
  ...
  const freeRegistration = launchMode === 'FREE_REGISTRATION';
  requireValue('NOTIFICATION_TRANSPORT', env.NOTIFICATION_TRANSPORT === (freeRegistration ? 'disabled' : 'line'), `NOTIFICATION_TRANSPORT must be ${freeRegistration ? 'disabled' : 'line'} for ${launchMode ?? 'the selected launch mode'}.`);
  ...
  requireValue('LINE_OAUTH_TRANSPORT', env.LINE_OAUTH_TRANSPORT === (freeRegistration ? 'disabled' : 'line'), `LINE_OAUTH_TRANSPORT must be ${freeRegistration ? 'disabled' : 'line'} for ${launchMode ?? 'the selected launch mode'}.`);
  requireValue('BILLING_TRANSPORT', env.BILLING_TRANSPORT === (freeRegistration ? 'disabled' : 'stripe'), `BILLING_TRANSPORT must be ${freeRegistration ? 'disabled' : 'stripe'} for ${launchMode ?? 'the selected launch mode'}.`);
  requireValue('STRIPE_LIVE_MODE', env.STRIPE_LIVE_MODE === (freeRegistration ? 'false' : 'true'), `STRIPE_LIVE_MODE must be ${freeRegistration ? 'false' : 'true'} for ${launchMode ?? 'the selected launch mode'}.`);
  ...
  manual.push({ code: 'DATABASE_RUNTIME_ROLE', message: 'Verify that DATABASE_URL uses the restricted runtime role.' });
  manual.push({ code: 'LEGAL_RELEASE', message: 'Confirm that reviewed legal documents are published in the application.' });
  manual.push({ code: 'PROVIDER_LIVE_TESTS', message: `Complete Supabase, Resend, and Turnstile live tests${freeRegistration ? '.' : ', plus LINE and Stripe live tests.'}` });
  return { service, launchMode, ok: errors.length === 0, errors, manual };
}
