...
test('staff schedules and cancels a target-race announcement on mobile-ready UI', async ({ page }, testInfo) => {
  ...
  const card = page.locator('.schedule-race').filter({ hasText: race.name }); await expect(card).toBeVisible(); await card.getByLabel('公開日時（JST）').fill(`${raceDate}T14:30`); await card.getByLabel('予約理由').fill('E2Eで定刻配信を確認'); await card.getByRole('button', { name: '予約する' }).click();
