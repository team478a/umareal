...
const draftFor = (entryId: string, visibility: 'FREE' | 'PAID' = 'FREE'): PredictionDraft => ({ ...emptyPredictionDraft, visibility, confidence: 'A', summary: '結合試験の最終見解', marks: [{ entryId, mark: 'HONMEI', reason: '総合評価' }] });
...
describe('prediction drafts, publication and immutable versions', () => {
  ...
  it('keeps detailed horse information restricted even when legacy visibility says FREE', async () => {
    ...
    expect(anonymousPaidFirst.body.latest.summary).toBeUndefined(); expect(anonymousPaidFirst.body.versions[0].locked).toBe(true); expect(anonymousPaidFirst.body.versions[1].locked).toBe(true); expect(anonymousPaidFirst.body.versions[1].summary).toBeUndefined(); expect(anonymousPaidFirst.body.versions[1].correctionReason).toBeUndefined();
  ...
  it('rejects stale previews and enforces the deadline inside PostgreSQL', async () => {
    ...
    await expect(db.predictionVersion.create({ data: { predictionId: prediction.id, version: 1, status: 'PUBLISHED', visibility: 'FREE', confidence: 'A', stance: null, summary: 'deadline bypass', estimatedTotalYen: null, formatVersion: 'HORSE_EVALUATION_V1', contentSnapshot: {}, assessmentSnapshot: [], publisherId: late.owner.user.id, deadlineAt: new Date(Date.now() + 3600000) } })).rejects.toThrow();
